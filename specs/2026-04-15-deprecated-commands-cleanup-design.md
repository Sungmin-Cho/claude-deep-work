...
- **Status**: Draft v3 (post-plan-review revision, awaiting user review)
...
- **Review history**:
  ...
  - v2 (commit `bc08683`) — review 수용하여 삭제 범위 제거, 파급 범위 보강, 버전 bump 정책 조정.
  - v3 (현재) — plan review 수용: 커맨드 수 정정(22→23), README "What changed" 섹션·커맨드 본문 잔존 `/deep-report` 안내 범위 추가, Session Report 3위치 일관성 규칙 추가, 검증 grep의 spec/plan 자체 매치 제외.
...
총 23개 → 23개 유지. 라벨만 변경. (실측: `ls commands/*.md | wc -l` = 23)
...
**(d) README.md L568–L572 / README.ko.md L559–L563 "What changed" bullets** (v2 plan review 반영)
현재 문구가 여전히 "13 commands deprecated" 및 "`/deep-status` Replaces `/deep-report` ..." 식의 옛 서술을 남기고 있어 v6.2.1 분류와 충돌. 다음으로 교체:
- `13 commands deprecated` → `13 commands reclassified into Quality Gate / Internal / Escape hatch / Utility (v6.2.1)`
- `Replaces /deep-report, /deep-receipt, /deep-history, /deep-assumptions with flags` → `Routes --report / --receipts / --history / --assumptions flags to the same implementations as the standalone commands (manual invocation remains supported)`
(ko 동일 번역)

**(e) 커맨드 본문의 잔존 `/deep-report` 안내** (v2 plan review 반영)
- `commands/deep-report.md:271` "리포트를 검토하고 필요시 /deep-report 로 재생성할 수 있습니다." → "리포트를 검토하고 필요 시 `/deep-status --report` 또는 `/deep-report`로 재생성할 수 있습니다."
- `commands/deep-resume.md:82` "리포트 확인: /deep-report" → "리포트 확인: `/deep-status --report` (또는 `/deep-report`)"
- `/deep-report`를 Internal로 분류하지만 수동 경로 공식 인정(§3 D-1)하므로 `--report` 플래그와 standalone 이름을 **둘 다** 남긴다.
...
- `skills/deep-work-workflow/SKILL.md` L256 `Manual: /deep-report or /deep-status --report` **유지** (§3 D-1의 "수동 경로 공식" 정책과 일치). Session Report 제목(L253)도 "both supported"로 표현하여 3위치(제목·본문·커맨드 본문) 일관성 확보.

### 4.5. CHANGELOG
`CHANGELOG.md`, `CHANGELOG.ko.md`에 v6.2.1 섹션 신설. 요지:
- Reclassified: "Deprecated in v5.2" 블록을 가진 11개 커맨드와 같은 표에 함께 분류되었던 2개(`deep-brainstorm`, `deep-phase-review`)를 5개 카테고리(Quality Gate / Internal / Escape hatch / Utility / Special utility)로 재분류.
...
**삽입 위치 규칙**: Keep a Changelog 관례를 지키기 위해 intro 문구(`All notable changes...`, `The format is based on [Keep a Changelog]...`) 아래, 최신 릴리즈 섹션(`## [6.2.0] — 2026-04-14`) 바로 **위**에 삽입한다. `# Changelog` 헤더 바로 아래 삽입은 intro 문구와 릴리즈 섹션이 섞이므로 금지.
...
15. **검증 체크리스트**:
    ...
    - `grep -rn "Deprecated in v5.2" commands/ skills/ README.md README.ko.md` → 결과 0건 (spec/plan 자체 파일 제외).
    - `grep -rn "Deprecated commands (13)\|Deprecated 커맨드 (13개)" skills/ README.md README.ko.md` → 결과 0건.
    - `grep -Ern "/deep-cleanup|/deep-resume" commands/ skills/ README.md README.ko.md CHANGELOG.md CHANGELOG.ko.md` → 본 스펙이 정의한 위치에만 남아있는지 확인.
    - 추가 sweep: `grep -rn "/deep-report\|/deep-assumptions" commands/ skills/` → hook 문구 갱신 누락 없는지, 그리고 Internal 커맨드 본문에 남은 안내 문구가 `/deep-status --report (또는 /deep-report)` 형식(양쪽 경로 표기)으로 통일되어 있는지 확인.
