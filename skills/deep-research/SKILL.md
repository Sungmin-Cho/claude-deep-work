...
2. fitness.json 미존재 시 → 생성 제안 (AskUserQuestion)
...
1. Read `../shared/templates/research-template.md` — 구조 템플릿 로드.
2. Section 2의 6개 영역 분석 결과로 placeholder 전부 치환.
3. [RF-NNN] / [RA-NNN] 태그는 단조 증가. 증분 리서치(`--incremental`) 시 기존 태그 보존 + 신규 findings에 다음 번호 부여.
4. Write `$WORK_DIR/research.md`.

**Tag 규칙**: 삭제 전 plan.md 참조 여부 확인.
...
4. **진행 모니터링**: TaskList로 진행 확인 + 완료 표시
