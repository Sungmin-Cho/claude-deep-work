...
1. **Claude Code 슬래시** — 사용자가 `/deep-fork [args...]` 입력 (skill 의 `user-invocable: true` 가 슬래시 진입을 허용).
2. **타 에이전트 / Codex / Copilot CLI / Gemini CLI / SDK** — `Skill({ skill: "deep-work:deep-fork", args: "..." })` 형태로 명시 invoke (cross-platform 표준 경로).
...
| `--from-phase=<phase>` | `brainstorm|research|plan|implement` 중 재시작 phase 강제 |
