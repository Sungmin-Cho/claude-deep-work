...
3. **Suggest (absent)** — emit a one-line note: *"No `.deep-memory/latest-brief.md` found. Run `/deep-memory-brief \"<task>\"` first if you want cross-project recall."* Then continue normally — no `AskUserQuestion`, no auto-invoke. **The user must explicitly request recall.**
