...
**Tech Stack:** Node 20+ (zero-dep, `node --test`), YAML frontmatter state, 마크다운 SKILL.md 프롬프트 배선.
...
- Node 20+ / zero-dependency. 테스트는 `node --test`(개별) / `npm test`(전체, `test:all` glob이 `runtime/**/*.test.js`·`scripts/**/*.test.js`를 자동 포함).
...
unknown 런타임 중복 경고는 첫 1건만 남기도록 `warnings = [...new Set(warnings)]`로 dedupe 후 반환. `module.exports`에 `decideModelRouting` 추가.
...
`:32` PRESET defaults 상수의 `model_routing` 배열도 `['      model_routing: auto']`로, `:34`의 interactive 목록에서 `'      - model_routing',` 제거.
...
- [ ] **Step 1: 실패하는 테스트 추가** — `scripts/migrate-model-routing.test.js`
...
`migrateStateFile` 내 파일 read 직후(치환 로직 전)에:

```js
if (/^model_routing_meta:/m.test(text)) {
...
> **v6.10.0**: 순회 전 `filterAskItems()`(recommender-runtime)를 적용한다 — 구프로필 `interactive_each_session`에 `model_routing`이 남아 있어도 ask하지 않는다(모델은 §1-8-6에서 자동 결정).

- [ ] **Step 3: orchestrator 신규 §1-8-6 — 자동 모델 결정 (state 작성 직전)**

§1-8과 §1-9 사이에 신규 섹션:

````markdown
## 1-8-6. 자동 모델 결정 (v6.10.0)
...
git add skills/deep-work-orchestrator/SKILL.md skills/deep-implement/SKILL.md \
  skills/deep-resume/SKILL.md skills/shared/references/model-routing-guide.md
git commit -m "$(cat <<'EOF'
...
1. **Spec coverage**: 설계 §1 컴포넌트 표 12행 전부 태스크 매핑 확인 — model-catalog(T1), detect-runtime(T2), model-routing-runtime(T3-5), model-routing-cli(T6), session-recommender+recommender-runtime(T7), flags-runtime(T8), profile-runtime(T9), migrate 가드(T10), orchestrator/deep-implement/deep-resume/guide(T11), codex pin(T12), 버전/CHANGELOG(T13). 리뷰 Medium-1(§2.5→T4/T11), Medium-2(→T10), Medium-3(→T7/T11) 해소 태스크 존재.
