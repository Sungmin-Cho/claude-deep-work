...
- Produces (Task 2·3·4가 의존):
  - `decideRiskProfile({ stage, taskText, signals, evidence, priorProfile }) → { stage, class, score, confidence, dimensions, hard_triggers, rationale, input_digest, transition }` — `decided_at`은 반환하지 않음(CLI가 부착)
...
test('decideRiskProfile — 결정론 + digest (시각 필드 없음)', () => {
  ...
  assert.match(a.input_digest, /^sha256:[0-9a-f]{64}$/);
...
function decideRiskProfile({ stage, taskText, signals, evidence, priorProfile } = {}) {
  ...
  const input_digest = canonicalDigest({ stage: effectiveStage, taskText: taskText ?? '',
    signals: signals ?? {}, evidence: evidence ?? {} }); // 시각 필드·priorProfile 제외 (§4.1)
  return { stage: effectiveStage, class: cls, score, confidence,
    dimensions: scored.dimensions, hard_triggers: hardTriggers,
    rationale: scored.rationale, input_digest, transition };
}
...
```js
'use strict';
const { TIERS } = require('./model-catalog.js');
const { CLASS_ORDER } = require('./risk-runtime.js');
...
- Produces (Task 6·7·8 스킬이 의존):
  ...
  - `risk_profile`에는 CLI가 `decided_at`(ISO 8601) 부착, `policy_snapshot`에는 `compiled_at`+`based_on` 부착
  ...
  try {
    ...
    if (args.workDir) {
      try {
        ...
        inputRef = { path: artifactPath, digest: canonicalDigest(effective) };
...
const FIXTURES = [
  ...
  { name: 'API endpoint 추가', input: { stage: 'authoritative', taskText: 'add REST endpoint',
    evidence: { changed_paths: ['src/api/routes.js', 'src/api/handler.js'],
      keywords: ['public api', 'interface 변경'], side_effects: [], evidence_refs: [] } },
  ...
  { name: '테스트 없는 legacy 모듈', input: { stage: 'authoritative', taskText: 'legacy 모듈 리팩터',
    ...
    evidence: { changed_paths: ['legacy/a.js', 'legacy/b.js', 'legacy/c.js', 'legacy/d.js', 'legacy/e.js'],
      keywords: ['refactor', '전체'], side_effects: [], evidence_refs: [] } },
...
- [ ] **Step 2: 실행 — 실패하는 fixture가 있으면 렉시콘 조정**

Run: `node --test tests/risk-fixture-matrix.test.js`
Expected: 실패 fixture의 진단 메시지(score/triggers)를 보고 `runtime/risk-runtime.js`의 렉시콘·trigger 정규식을 조정한다. **fixture 기대값은 조정하지 않는다** (Global Constraints). 조정 후 `node --test runtime/risk-runtime.test.js`도 함께 green 확인.
...
2. state에서 추출: `model_routing` 블록 → `model_routing`, `model_routing_meta.tiers` → `tiers`, `model_routing_meta.pinned` → `pinned`(없으면 `{}`), `risk_profile_json`의 `provisional.class` → `prior_profile: {"class": "<값>"}`.
...
`tests/methodology-shadow-receipt.test.js` — **동일 fixture에 `methodology_shadow` 유/무 두 번 `validate-receipt.sh`를 실행해 결과가 동일**함을 확인한다 (실제 리더군 기준의 forward-compat, 스펙 §8.3):
...
function runValidator(payloadExtra) {
...
test('methodology_shadow 유/무가 validate-receipt.sh 판정을 바꾸지 않는다 (§8.3)', () => {
  const without = runValidator({});
  const withShadow = runValidator({ methodology_shadow: { schema_version: 1,
...
Expected: PASS. 실패 시 validate-receipt.sh의 요구 필드(출력 메시지 기준)를 fixture `payload`에 보강한다 — **두 실행의 유일한 차이는 methodology_shadow 유/무**라는 불변식만 유지하면 됨. validate-receipt.sh 자체는 수정 금지.
