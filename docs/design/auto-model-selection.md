...
| `runtime/recommender-runtime.js` | 수정 | KEYS/파서 4-key + difficulty 검증. 구버전 5-key 응답 관용 파싱(model_routing 무시) |
| `scripts/parse-deep-work-flags.js` | 수정 | `--model-routing="implement=opus,test=haiku"` 플래그 추가(allowlist 검증) |
| `skills/deep-work-orchestrator/SKILL.md` | 수정 | §1-4 ask에서 model_routing 제거, §1-9 직전 엔진 실행 + state 기록, §1-11 자동 결정 결과·근거 표시 |
| `skills/deep-resume/SKILL.md` | 수정 | resume 시 `model_routing_meta.runtime` ≠ 현재 runtime → tier에서 재해석 + state 갱신 + 1회 안내 |
| `skills/deep-implement/SKILL.md` | 수정 | 기존 slice-size auto(S→haiku 등)를 tier 표현(S→light, M/L→standard, XL→deep)으로 치환 후 카탈로그 해석 |
...
세션 init (orchestrator §1-9 state 작성 직전):
  ...
  state.model_routing      = resolved               # concrete 모델명 — 기존 소비 경로 무변경
...
1. CLI `--model-routing="implement=opus,..."` (명시 k=v)
...
- **override**: ① `--model-routing="k=v,..."` 플래그(값 allowlist: haiku/sonnet/opus/main + light/standard/deep tier명 허용 — tier명이면 카탈로그 해석) ② `/deep-slice model` 유지 ③ 프로필 pinned 값 존중.
- recommender 실패 시에도 질문으로 되돌아가지 않는다 — baseline 자동값 + "(자동 추천 실패 — 기준선 적용)" 1회 표시.

## 5. 호환성 & 마이그레이션

- **프로필 v3**: 신규 프로필 defaults는 `model_routing: auto`(스칼라 sentinel — ENUMS에 이미 존재하는 `auto` 재사용).
...
- **기존 migration**(v6.4.0 `main→sonnet`, `scripts/migrate-model-routing.js`)은 pinned 값에만 계속 적용, 무수정.
...
- `scripts/parse-deep-work-flags.test.js`류 (갱신): `--model-routing` k=v allowlist.
