...
- `reviewers[]` 항목: `{ role: 'structural'|'semantic'|'executability', channel: 'subagent'|'codex-cli'|'gemini-cli'|'deep-review', tier, effort, required: bool }`
  - `tier`는 부록 B.1의 리뷰어 tier 표에서, `effort`는 v6.11 `EFFORT_CATALOG`(정본 유지)에서 가져온다. concrete 모델 해석은 소비 시점에 `resolveTier(tier, runtime)`(기존 `runtime/model-catalog.js`)로 수행 — **런타임 교차 유출 없음** 기존 보장 재사용.
